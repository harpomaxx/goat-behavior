...
library(ztable)
options(ztable.type='viewer')
r<-readr::read_csv("../../../results-resample.csv")
r <- r %>%  as.data.frame() %>% select(c(1,2,3,4,5,6,7))
names(r)<-c("exp","BAcc","BAcc_sd","Sens","Sens_sd","Spec","Spec_sd")
r<- r %>% filter(!is.na(exp))
ztable(r) %>% makeHeatmap(c()) 
```
